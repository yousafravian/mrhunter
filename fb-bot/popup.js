document.getElementById('activate').addEventListener('click', () => {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs.length > 0) {
      ...
      chrome.runtime.sendMessage({ action: 'fetchData', tabId: tabId }, (response) => {
        ...
        if (chrome.runtime.lastError) {
          ...
          output.textContent = 'Success: Data fetched and sent to content script.';
        } else {
          console.error('Error:', response ? response.error : 'No response received');
          output.textContent = 'Error: ' + (response ? response.error : 'No response received');
        }
      });
    }
  });
