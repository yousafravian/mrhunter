...
async function getAllProfileLinks(startNum, foundProfiles) {
  ...
  while (true) {
    ...
    if (profile && !foundProfiles.has(profile.href)) {
      ...
      await sendProfileLinksToServer(profile.href, pageDomain);
...
async function sendProfileLinksToServer(profileLink, pageDomain) {
  try {
...
async function extractProfileLinks() {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "startScraping") {
      ...
      console.log("fired from page script");
